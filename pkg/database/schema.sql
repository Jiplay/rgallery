...
CREATE VIRTUAL TABLE IF NOT EXISTS images_virtual
  USING FTS5(
      hash,
      path,
      subject,
...
CREATE TRIGGER IF NOT EXISTS images_delete AFTER DELETE ON media BEGIN
      DELETE FROM images_virtual WHERE hash = OLD.hash;
    END;
